...
# 
# Description:  This program uses the preprocessed_data.csv file generated by 2_preprocessing.R (and the one generated by 2b_genderedSubsets.R if M/F analysis selected)
#               2_preprocessing.R (and 2b_genderedSubsets.R) haas to be run before this program is executed
#                 preprocessed_data.csv (or preprocessed_data_females.csv or preprocessed_data_males.csv) is read as the dataFrame object named data.
# 
...
repeat{
  #beepr::beep(sound = 10)   #notify user to provide input
  userSubsetSelection <- readline(prompt="Enter '1' for all learners, '2' or 'f' for female learners, '3' or 'm' for male learners: ");
  ...
  if(userSubsetSelection == 1){  #dataset: all learners
    ...
    #check for user access datafile existance
  ...
  else if(userSubsetSelection == "f" || userSubsetSelection == "F" || userSubsetSelection == 2){  #dataset: female learners
...
#save the appropiate access data CSV
...
if(clusterTypeSelection==1)
{
  ...
  ##TW:ver:the following should be unnecessisary
  ...
  for(i in 2:9)
  {
    if(gap_values[i-1]<gap_values[i] & gap_values[i+1]<gap_values[i])
      # {			##TW: ??: ask DR why this was removed and only one recomendation is now being provided.  If this is removed then why even solicit user input on number of clusters?  If it is removed then delete the extra variables above the "for".
  ...
  for(k in cluster_order)
  {
    ...
    for(j in 1:nrow(temp))
    {
      ...
      ##TW:??: ask DR why the colors end up randomized.  I think it'd be better if they were consistant accross graphs
      if(k==1)
      {
...
} else if(clusterTypeSelection==2)
{
  ...
  {
    if(gap_values[i-1]<gap_values[i] & gap_values[i+1]<gap_values[i])
      # {    #TW:??: same question for DR as above in k-means section.  Why eleminate the two calculated.  Why give user the choice?
      # optimal_clusters=c(optimal_clusters,i)
      # counter <- counter+1
      # }
      # if(counter==2)
